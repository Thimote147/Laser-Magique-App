...
class BookingConsumptionController {
  ...
  static void preInitializePriceService(BuildContext context, String bookingId) {
    try {
      ...
      if (cachedTotal > 0) {
        ...
        debugPrint('BookingConsumptionController: Pre-initialized price service with $cachedTotal for booking $bookingId');
      }
    } catch (e) {
  ...
  Future<void> initialize(BuildContext context) async {
    ...
    try {
      ...
      if (cachedTotal > 0) {
        ...
        debugPrint('BookingConsumptionController: Pre-sync with cached total $cachedTotal for booking $bookingId');
      }
      
      // Charger les consommations depuis le StockViewModel
      final consumptions = await stockVM.getConsumptionsWithStockItems(bookingId);
      ...
      if (finalTotalAmount != cachedTotal) {
        ...
        debugPrint('BookingConsumptionController: Final sync with total $finalTotalAmount for booking $bookingId');
      }
  ...
  ValueNotifier<int> getQuantityNotifier(String consumptionId, int initialQuantity) {
  ...
  void updateTotal(double newTotal) {
    if (totalAmountNotifier.value != newTotal) {
      ...
      debugPrint('BookingConsumptionController: Updated total to $newTotal for booking $bookingId');
    }
  }
  ...
  Future<void> updateConsumptionQuantity(
    ...
    try {
      ...
      final quantityNotifier = getQuantityNotifier(consumptionId, consumption.quantity);
      quantityNotifier.value = newQuantity;
      
      // Créer une nouvelle liste avec la quantité mise à jour
      final updatedConsumptions = List<(Consumption, StockItem)>.from(currentConsumptions);
      ...
      final consumption = currentConsumptions.firstWhere(
        ...
        ), StockItem(id: '', name: '', category: '', price: 0.0, quantity: 0, alertThreshold: 0)),
      );
  ...
  Future<void> deleteConsumption(
    ...
    try {
      ...
      final updatedConsumptions = currentConsumptions
      ...
      debugPrint('BookingConsumptionController: Deleted consumption $consumptionId for booking $bookingId');
  ...
  Future<void> forceSyncWithPriceService(BuildContext context) async {
    try {
      ...
      final cachedTotal = stockVM.getConsumptionTotal(bookingId);
      final currentServiceTotal = _priceService.getNotifierForBooking(bookingId).value;
      
      // Ne synchroniser que si les valeurs sont différentes
      if (cachedTotal > 0 && cachedTotal != currentServiceTotal) {
        totalAmountNotifier.value = cachedTotal;
        _priceService.updateConsumptionPriceSync(bookingId, cachedTotal);
        debugPrint('BookingConsumptionController: Force sync - updated price service with total $cachedTotal for booking $bookingId');
